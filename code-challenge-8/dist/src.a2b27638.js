...
var calContainer = document.querySelector('.js-cal'),
...
function handleEqual() {
  var equalResult = eval(tempResult);
  if (equalResult) result.innerHTML = equalResult;else result.innerHTML = 0;
  opNum = 0;
}

resetBtn.addEventListener('click', resetResult);
numBtn.forEach(function (num) {
  return num.addEventListener('click', handleNum);
});
opBtn.forEach(function (op) {
  return op.addEventListener('click', handleOp);
});
